...
export default function ReceiptGenerationScreen({ onClose, transactionData }: ReceiptGenerationScreenProps) {
  ...
  const descriptionOptions = ["capu利用料", "ポイント購入代", "サービス利用料", "その他"]
  ...
  const handleGenerate = () => {
    ...
    // Auto close popup after 2 seconds and then close the screen
    setTimeout(() => {
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full p-4 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-pink-400 focus:border-pink-400"
              placeholder=""
            />
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <button
                ...
                className="w-full p-4 bg-white border border-gray-200 rounded-lg text-sm text-left flex items-center justify-between focus:outline-none focus:ring-2 focus:ring-pink-400 focus:border-pink-400"
              >
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg p-6 mx-4 max-w-sm w-full shadow-xl">
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">発行完了</h3>
              <p className="text-sm text-gray-600">
                領収書の発行が完了しました。
              </p>
              {formData.email && (
                <p className="text-sm text-gray-600 mt-2">
                  メールアドレスに領収書のリンクをお送りしました。
                </p>
              )}
            </div>
          </div>
