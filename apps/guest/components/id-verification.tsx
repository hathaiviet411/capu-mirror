...
export default function IdentityVerificationScreen({ onBack, onSubmit }: IdentityVerificationScreenProps) {
  ...
  const isUnderReview = useMemo(() => existingVerification?.status === "UNDER_REVIEW", [existingVerification?.status])
  const isApproved = useMemo(() => existingVerification?.status === "APPROVED", [existingVerification?.status])
  const isRejected = useMemo(() => existingVerification?.status === "REJECTED", [existingVerification?.status])
  const isExpired = useMemo(() => existingVerification?.status === "EXPIRED", [existingVerification?.status])

  const isFormDisabled = useMemo(() => isUnderReview || isApproved, [isUnderReview, isApproved])
  ...
  const handleDocumentTypeModalOpen = useCallback(() => {
    if (!isFormDisabled) {
      setShowDocumentTypeModal(true)
    }
  }, [isFormDisabled])

  const handleDocumentTypeModalClose = useCallback(() => {
  ...
  }, [isFormDisabled, uploadedFiles.length, toast])
  ...
  }, [uploadedFiles, documentType, submitVerificationMutation, onSubmit, toast])

  const handleRemoveFileFinal = useCallback((fileId: string) => {
    handleRemoveFile(fileId)
  }, [handleRemoveFile])

  const handleDocumentTypeSelectFinal = useCallback((type: DocumentTypeKey) => {
    handleDocumentTypeSelect(type)
  }, [handleDocumentTypeSelect])

  const handleRemoveFileClick = useCallback((fileId: string) => {
    return () => handleRemoveFileFinal(fileId)
  }, [handleRemoveFileFinal])

  const handleDocumentTypeSelectClick = useCallback((type: DocumentTypeKey) => {
    return () => handleDocumentTypeSelectFinal(type)
  }, [handleDocumentTypeSelectFinal])
      ...
      {showDocumentTypeModal && (
        <div className="fixed inset-0 z-50 bg-white w-full md:max-w-sm mx-auto flex flex-col">
          <div className="bg-gold-pink-gradient px-4 py-4 h-16 flex items-center gap-3 border-b shadow-lg flex-shrink-0">
            <button onClick={handleDocumentTypeModalClose}>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="space-y-2">
              {documentTypes.map((type) => (
                <button
                  key={type}
                  onClick={handleDocumentTypeSelectClick(type)}
